...
def get_api_key(username, role):
    ...
    auth_api = conjur.api.authn_api.AuthnApi(get_api_client())
...
def get_api_client(username='admin', role='user'):
    ...
    auth = conjur.api.AuthnApi(client)
...
def setup_oidc_webservice():
    ...
    account = os.environ[CONJUR_ACCOUNT]
    policy_api = conjur.api.policies_api.PoliciesApi(client)
    with open(OIDC_POLICY_FILE, 'r') as policy_file:
        policy = policy_file.read()
    policy_api.update(account, 'root', policy)

    secrets_api = conjur.api.secrets_api.SecretsApi(client)
    secrets_api.create(
        account,
        'variable',
...
class ConfiguredTest(unittest.TestCase):
    ...
    def load_default_policy(cls):
        ...
        policy_api.replace(cls.account, 'root', default_policy)
